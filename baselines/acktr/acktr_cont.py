...
from baselines import common, logger
...
def pathlength(path):
    return path["reward"].shape[
        0]  # Loss function that we'll differentiate to get the policy gradient
...
def learn(env,
    ...
    stepsize = tf.Variable(
        initial_value=np.float32(np.array(0.03)), name='stepsize')
    ...
    update_op, q_runner = optim.minimize(
        loss, loss_sampled, var_list=pi_var_list)
