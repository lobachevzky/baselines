...
from baselines.a2c.utils import (EpisodeStats, Scheduler, avg_norm, batch_to_seq, cat_entropy_softmax, check_shape,
                                 find_trainable_variables, get_by_index, gradient_add, make_path, q_explained_variance,
                                 seq_to_batch)
...
class Model(object):
    def __init__(self, policy, ob_space, ac_space, nenvs, nsteps, nstack,
        ...
        train_model = policy(
            sess, ob_space, ac_space, nenvs, nsteps + 1, nstack, reuse=True)
        ...
        adv_bc = (
            q - tf.reshape(v, [nenvs * nsteps, 1]))  # [nenvs * nsteps, nact]
        logf_bc = tf.log(f + eps)  # / (f_old + eps)
        check_shape([adv_bc, logf_bc], [[nenvs * nsteps, nact]] * 2)
        gain_bc = tf.reduce_sum(
            logf_bc * tf.stop_gradient(
                adv_bc * tf.nn.relu(1.0 - (c / (rho + eps))) * f),
        ...
        ev = q_explained_variance(
            tf.reshape(q_i, [nenvs, nsteps]), tf.reshape(
                qret, [nenvs, nsteps]))
        ...
        if trust_region:
            g = tf.gradients(
                -(loss_policy - ent_coef * entropy) * nsteps * nenvs,
                f)  # [nenvs * nsteps, nact]
            ...
            adj = tf.maximum(
                0.0, (tf.reduce_sum(k * g, axis=-1) - delta) / (tf.reduce_sum(
                    tf.square(k), axis=-1) + eps))  # [nenvs * nsteps]
...
class Runner(object):
    ...
    def run(self):
        enc_obs = np.split(
            self.obs, self.nstack, axis=3)  # so now list of obs steps
        mb_obs, mb_actions, mb_mus, mb_dones, mb_rewards = [], [], [], [], []
        for _ in range(self.nsteps):
            actions, mus, states = self.model.step(
...
def learn(policy,
    ...
    if replay_ratio > 0:
        buffer = Buffer(
            env=env, nsteps=nsteps, nstack=nstack, size=buffer_size)
    else:
        buffer = None
    nbatch = nenvs * nsteps
