...
class Model(object):
    def __init__(self, policy, ob_space, ac_space, nenvs, nsteps, ent_coef,
        ...
        train_ob_placeholder = tf.placeholder(
            ...
            shape=(nenvs * (nsteps + 1), ) + ob_space.shape)
        with tf.variable_scope('acer_model', reuse=tf.AUTO_REUSE):

            step_model = policy(
                observ_placeholder=step_ob_placeholder, sess=sess)
            train_model = policy(
                observ_placeholder=train_ob_placeholder, sess=sess)
        ...
        with tf.variable_scope(
                "acer_model", custom_getter=custom_getter, reuse=True):
            polyak_model = policy(
                observ_placeholder=train_ob_placeholder, sess=sess)
        ...
        self.train = train
        self.save = functools.partial(
            save_variables, sess=sess, variables=params)
