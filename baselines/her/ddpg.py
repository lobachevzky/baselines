...
from baselines.her.util import (convert_episode_to_batch_major, flatten_grads, import_function, store_args,
...
class DDPG(object):
    ...
    def _random_action(self, n):
        return np.random.uniform(
            low=-self.max_u, high=self.max_u, size=(n, self.dimu))
        ...
        for epsd in range(
            ...
            for transition in range(self.T):
                ...
                for idx, key in enumerate(info_keys):
                    info_values[idx][transition, i] = demoData['info'][epsd][
                        transition][key]
            ...
            print("Demo buffer size currently ", demoBuffer.
                  get_current_size())  #print out the demonstration buffer size
    ...
    def stage_batch(self, batch=None):
        ...
        self.sess.run(
            self.stage_op, feed_dict=dict(zip(self.buffer_ph_tf, batch)))
    ...
    def _create_network(self, reuse=False):
        ...
        self.Q_grad_tf = flatten_grads(
            grads=Q_grads_tf, var_list=self._vars('main/Q'))
        self.pi_grad_tf = flatten_grads(
            grads=pi_grads_tf, var_list=self._vars('main/pi'))

        # optimizers
        self.Q_adam = MpiAdam(self._vars('main/Q'), scale_grad_by_procs=False)
        self.pi_adam = MpiAdam(
            self._vars('main/pi'), scale_grad_by_procs=False)
        ...
        self.update_target_net_op = list(
            map(
                lambda v: v[0].assign(self.polyak * v[0] + (1. - self.polyak) * v[1]),
                zip(self.target_vars, self.main_vars)))
